...
export function Navbar() {
  ...
  const menuItems: MenuItem[] = [
    ...
    { label: 'BLOG', href: 'https://blog.leosenderovsky.com.ar', external: true },
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 bg-gradient-to-r from-[#766dff] to-[#88f3ff] ${isScrolled ? 'shadow-lg py-2' : 'py-4'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        <Link to="/" className="z-50" onClick={() => window.scrollTo(0, 0)}>
          <img src="/assets/img/logo.png" alt="Leo Senderovsky" className={`transition-all duration-300 object-contain ${isScrolled ? 'h-12 max-h-[75px]' : 'h-16 max-h-[75px]'}`} />
        </Link>
                ...
                <AnimatePresence>
                  {openSubmenu === item.label && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-full left-0 mt-2 bg-gradient-to-r from-[#766dff] to-[#88f3ff] shadow-lg rounded-md py-2 min-w-48 z-50"
                    >
                      {item.submenu.map((subitem, subidx) => (
                        subitem.submenu ? (
                          <div key={subidx} className="relative group">
                            <Link
                              to={subitem.path!}
                              className="block px-4 py-2 text-[#000b77] hover:bg-white/30 font-heading uppercase text-[20px]"
                            >
                              {subitem.label}
                            </Link>
                            <div className="absolute left-full top-0 mt-0 bg-gradient-to-r from-[#766dff] to-[#88f3ff] shadow-lg rounded-md py-2 min-w-48 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="absolute top-full left-0 w-full bg-gradient-to-r from-[#766dff] to-[#88f3ff] shadow-xl py-6 flex flex-col items-center space-y-4 lg:hidden"
            >
              {menuItems.map((item, idx) => (
                item.submenu ? (
                                ...
                                <div className="ml-4 space-y-1">
                                  {subitem.submenu.map((subsubitem, subsubidx) => (
                                    <Link
                                      ...
                                      to={subsubitem.path!}
                                      onClick={() => setIsOpen(false)}
                                      className="block font-heading text-base uppercase tracking-wider"
                                      style={{ color: '#000b77' }}
                                    >
                                      {subsubitem.label}
                                    </Link>
